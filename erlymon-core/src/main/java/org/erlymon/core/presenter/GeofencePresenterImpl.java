...
public class GeofencePresenterImpl implements GeofencePresenter {
    ...
    private GeofenceView view;
    private Subscription subscription = Subscriptions.empty();

    public GeofencePresenterImpl(Context context, GeofenceView view) {
        this.view = view;
    ...
    public void onSaveButtonClick() {
        ...
        if (view.getGeofenceId() == 0) {
            subscription = model.createGeofence(view.getGeofence())
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .doOnNext(geofence -> realmdb.executeTransactionAsync(realm -> realm.copyToRealmOrUpdate(geofence)))
                    .subscribe(new Observer<Geofence>() {
                        ...
                        @Override
                        public void onNext(Geofence data) {
                            view.showData(data);
                        }
                    });
        } else {
            subscription = model.updateGeofence(view.getGeofenceId(), view.getGeofence())
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .doOnNext(geofence -> realmdb.executeTransactionAsync(realm -> realm.copyToRealmOrUpdate(geofence)))
                    .subscribe(new Observer<Geofence>() {
